			...
			if (SCH_tasks_G[Index].pTask){
				if (--SCH_tasks_G[Index].Delay == 0){
					...
					if (SCH_tasks_G[Index].Period){
						SCH_tasks_G[Index].Delay = SCH_tasks_G[Index].Period; //schedule regular tasks to run againn
					}
				}
			}
	}
